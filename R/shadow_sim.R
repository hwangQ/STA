# A function to simulate a minimum length of Shadow CAT by constructing two parallel shadow tests or 
# three parallel tests (when the number of test forms is three, then difference theta estimates are used using SE)  
...
ShadowPreSim <- function(item.pool, min.test, true.theta, ini.theta=0, lowb.content, a.strat=TRUE, 
					n.strat, breaks=NULL, ransq=TRUE, par.ransq, inter.method, D, lp.control) {
	...
	# constraint: test length
	lpSolveAPI::add.constraint(lprec=shadow, xt=rep(1, I), type="=", rhs=min.test, indices=1:I)

	# constraints: content
	for(i in 1:length(NcMin)) {
		lpSolveAPI::add.constraint(lprec=shadow, xt=rep(1, length(Vc[[i]])), type=">=", rhs=NcMin[i], indices=Vc[[i]])
	}

	if(a.strat) {
		# constraints: a-stratification
		for(i in 1:(length(Na)-1)) {
			lpSolveAPI::add.constraint(lprec=shadow, xt=rep(1, length(Va[[i]])), type=">=", rhs=Na[i], indices=Va[[i]])
		}
	}
	...
	for(i in 1:min.test) {
		...
		if(a.strat) {
			...
			# are classified to the first a-stratum are selected among items in the Shadow test
			if(Level[i] < n.strat) {
				item_adm <- item_shadow[idx_astr[sim_opt == 1] == Level[i]]
			} else {
				item_adm <- item_shadow
			}
	
			# exclude the items already administered in the prevous stage among the possible candidate items 
			if(i > 1) item_adm <- item_adm[!item_adm %in% item_recod]
			...
			if(ransq) {
				...
				# select the final candidate items that have high information values at the current ability estimate using a randomesque method
				if(NaCum[Level[i]] - (i - 1) >= par.ransq[Level[i]]) {
					item_ran <- item_adm[rank(info_adm) %in% 1:par.ransq[Level[i]]]
				} else {
					item_ran <- item_adm
				}
				# randomly selet an item to be administered at the next stage
			...
			## randomesque method is not used
				...
				# a next item is selected only based on the maximum value of item information
				item_next <- item_adm[info_adm == max(info_adm)]
				if(length(item_next) > 1) item_next <- item_next[1]
	...
	rr <- list(record=list(theta=theta_recod, se=se_recod, item=item_recod, response=item_ans, shadow=shadow_recod), 
				final=list(theta.est=theta.est, se.est=se.est, test=item_df),
				metaInfo=list(true.theta=true.theta, ini.theta=ini.theta, D=D, min.test=min.test, lowb.content=lowb.content,
							a.strat=a.strat, n.strat=n.strat, breaks=breaks, ransq=ransq, par.ransq=par.ransq),
				estimation=list(method=inter.method$method, priorDist=inter.method$priorDist, priorPar=inter.method$priorPar,
								parInt=inter.method$parInt, range=inter.method$range)
				)
